...
def run_flow(run_dir, flow_to_test, result_dir):
    ...
    for flow_name in flow_to_test:
        # Reset DUT
        os.system("plink.exe -ssh -pw brcm123$ root@192.168.100.31 cd /root/Desktop/4387_FW/4387_18_10_336_REF; ./load.sh;")
        time.sleep(40)
        ...
        while True:
            if os.path.isdir("Log") or os.path.isdir("Result_LP"):
                ...
                break
        run_cmd = ["IQfactRun_Console.exe", '-run', flow_name, "-repeat", "5", "-exit"]
...
def main():
    ...
    *********************************************************************
    run_dir = os.getcwd()
    run_dir = r"C:\LitePoint\IQfact_plus\IQfact+_BRCM_4387_MPS_4.0.0.46\bin"

    **************************Set Up Scripts********************************
    """


    package_p_drive = r"\\lphqfiler1\Software\LitePoint\Silicon_Solutions\IQAutoBuild\projects\IQfact+_BRCM_4387_MPS"
    package_name = "IQfact+_BRCM_4387_MPS_4.0.0.56.zip"

    package_loc = r"C:\4387_setup\package_zip"  # package needs to be copied to package_loc

    flowfile_loc = r"C:\4387_setup\Flow"
    setupfile_loc = r"C:\4387_setup\Setup"
    ...
    result_dir = "_".join([path_list, "Test_Results"])  # 4387_MPS_4.0.0.56_Test_Results
